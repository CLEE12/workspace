...
<body bgcolor="<%=bodyback_c%>">
		...
		<table border="0" width="650" cellpadding="5" cellspacing="5"
			...
			<tr height="30">
				<td rowspan="6" width="150" align="center" valign="middle"><img
					src="../imageFile/<%=book.getBook_image()%>" border="0" width="150"
					height="200"></td>
			...
			<tr>
				<td>출판사 : <%=book.getPublishing_com()%></td>
			</tr>
			<tr>
				<td>출판일 : <%=book.getPublishing_date()%></td>
			</tr>
			<tr>
				<td>정가 : <s><%=NumberFormat.getInstance().format(book.getBook_price())%>원</s><br>
					<%
			...
			<tr>
				<td>수량 : <input type="text" size="5" name="buy_count" value="1">
					개 <%
					if (id.equals("not")) {
 ...
 %> <input type="hidden" name="book_id"
					value="<%=book_id%>"> <input type="hidden"
					name="book_image" value="<%=book.getBook_image()%>"> <input
					type="hidden" name="book_title" value="<%=book.getBook_title()%>">
					...
					value="<%=book.getBook_count()%>"> <input type="submit"
					value="장바구니에 담기"> <%
 	}
 	}
